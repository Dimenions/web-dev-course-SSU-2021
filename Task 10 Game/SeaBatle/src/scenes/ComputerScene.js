...
class ComputerScene extends Scene {
    ...
    update() {
        ...
        if (isUnderPoint(mouse, opponent.table)) {//Если мышка над полем соперника
            const cell = cells.find((cell) => isUnderPoint(mouse, cell));

            if (cell) {
                cell.classList.add("battlefield-item__active");//Подсвет клетки

                if (this.playerTurn && mouse.left && !mouse.pLeft) {//Хотим сделать выстрел
                    const x = parseInt(cell.dataset.x);
                    const y = parseInt(cell.dataset.y);

                    const shot = new ShotView(x, y);//Создаём выстрел
        ...
        if (this.playerTurn) {//Меняем статус
