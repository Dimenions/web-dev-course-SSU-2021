...
class BattlefieldView extends Battlefield {
    ...
    dock = null;
    polygon = null;
    showShips = true;//Для скрытия кораблей соперника

    cells = [];// хранит все tr элементы
    ...
    constructor(showShips = true) {
        ...
        for (let y = 0; y < 10; y++) {//Чертим клетку
            const row = [];
        ...
        for (let x = 0; x < 10; x++) {//Буквы над полем
        ...
        for (let y = 0; y < 10; y++) {//Цифры возле поля
    ...
    addShip(ship, x, y) {//Переопределяем родителя
        if (!super.addShip(ship, x, y)) {
            return false;
        }

        if (this.showShips) {//Добавляем в док
    ...
    removeShip(ship) {
        ...
        if (Array.prototype.includes.call(this.dock.children, ship.div)) {//Заимствуем метод
            ship.div.remove();//Удадяем из дока
        }

        return true;
    }

    isUnder(point) {//Находится ли точка над игровым полем
    ...
    addShot(shot) {
        ...
        const cell = this.cells[shot.y][shot.x];//Ищем клетку выстрела
        const cellRect = cell.getBoundingClientRect();
        const rootRect = this.root.getBoundingClientRect();
    ...
    removeShot(shot) {
        ...
        if (Array.prototype.includes.call(this.polygon.children, shot.div)) {//Провекра есть ли выстрел в полигоне
            shot.div.remove();
        }
